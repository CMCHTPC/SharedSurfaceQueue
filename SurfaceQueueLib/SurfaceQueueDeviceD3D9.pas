...
function TSurfaceQueueDeviceD3D9.CreateSharedSurface(Width: UINT; Height: UINT; format: TDXGI_FORMAT; out ppSurface; out phandle: THANDLE): HResult; stdcall;
var
...
function TSurfaceQueueDeviceD3D9.ValidateREFIID(const ID: TGUID): boolean; stdcall;
begin
    Result := IsEqualGUID(ID, IDirect3DTexture9);
end;

function TSurfaceQueueDeviceD3D9.OpenSurface(hSharedHandle: THANDLE; out ppUnknown; Width: UINT; Height: UINT; format: TDXGI_FORMAT): HResult; stdcall;
var
    D3D9Format: TD3DFORMAT;
begin
    ...
    Result := m_pDevice.CreateTexture(Width, Height, 1, D3DUSAGE_RENDERTARGET, D3D9Format, D3DPOOL_DEFAULT, IDirect3DTexture9(ppUnknown), @hSharedHandle);
    if (Result = S_OK) then
    begin
...
function TSurfaceQueueDeviceD3D9.UnlockSurface(pSurface: IUnknown): HResult; stdcall;
var
    pSurf: IDirect3DSurface9;
begin
    Result := pSurface.QueryInterface(IID_IDirect3DSurface9, pSurf);
